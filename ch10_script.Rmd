...
train %>% 
  ggplot(aes(age, income, color = treatment)) + 
  geom_point() + 
  theme_classic()
...
diff_train <- train %>%
  group_by(treatment) %>%
  summarise(avg_inc = mean(income)) %>%
  pivot_wider(id_cols = treatment:avg_inc, names_from = treatment, values_from = avg_inc) %>% 
...
virtual_train_resamples <- tibble(replicate_id = 1:1000, 
                            sample = map(1:1000,
                                         ~ rep_sample_n(train, 
                                                        size = 115, 
                                                        replace = TRUE, 
...
find_diff(virtual_train_resamples[[2]][[1]])
